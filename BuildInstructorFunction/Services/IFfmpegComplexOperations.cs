...
namespace BuildInstructorFunction.Services
{
    public interface IFfmpegComplexOperations
    {
        ...
        List<(string id, string ffmpegReference)> BuildLayerCommand(StringBuilder command, Clip clip, List<(string id, string ffmpegReference)> splitLayers, List<DisplayLayer> orderedDisplayLayers, string watermarkFilePath);
